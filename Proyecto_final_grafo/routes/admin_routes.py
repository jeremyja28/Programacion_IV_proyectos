...
from models.models import User, Ciudad, Ruta, Provincia, db
...
def dashboard():
    ...
    stats = {
        ...
        'rutas_activas': Ruta.query.filter_by(estado='activa').count(),
        'rutas_inactivas': Ruta.query.filter_by(estado='inactiva').count(),
    ...
    if request.method == 'POST':
        ...
        if not nombre or not provincia_id:
            ...
            try:
                ...
                ciudad.latitud = float(latitud) if latitud else None
                ciudad.longitud = float(longitud) if longitud else None
...
def nueva_ruta():
    ...
    if request.method == 'POST':
        ...
        elif not distancia or not costo:
            flash('Distancia y costo son obligatorios.', 'warning')
        else:
            try:
                ruta = Ruta(
    ...
    if request.method == 'POST':
        ...
        if not ciudad_origen_id or not ciudad_destino_id:
            ...
            try:
                ...
                ruta.distancia = float(distancia)
                ruta.costo = float(costo)
                ruta.tiempo_estimado = int(tiempo_estimado) if tiempo_estimado else None
                ruta.estado = estado
