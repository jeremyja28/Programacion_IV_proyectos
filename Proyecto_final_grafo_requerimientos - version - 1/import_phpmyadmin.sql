...
-- VERIFICACION FINAL
-- =========================================
SELECT '✅ IMPORTACION COMPLETADA EXITOSAMENTE!' AS resultado;
SELECT '🚀 Configuración Laragon Puerto 3307' AS configuracion;
SELECT 'Credenciales App: admin / admin123' AS credenciales;
SELECT CONCAT('✅ Ciudades: ', COUNT(*)) AS total_ciudades FROM ciudades;
SELECT CONCAT('✅ Rutas: ', COUNT(*)) AS total_rutas FROM rutas;
SELECT CONCAT('✅ Usuarios: ', COUNT(*)) AS total_usuarios FROM users;

-- Mostrar algunas rutas de ejemplo
SELECT 
    CONCAT('📍 ', co.nombre, ' → ', cd.nombre, ' (', r.costo, '$ - ', r.distancia, 'km)') AS rutas_ejemplo
FROM rutas r
