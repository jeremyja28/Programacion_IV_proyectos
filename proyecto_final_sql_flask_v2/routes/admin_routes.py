...
def eliminar_ciudad(id):
    ...
    try:
        # Eliminar todas las rutas asociadas a esta ciudad
        rutas_origen = ciudad.rutas_origen.all()
        rutas_destino = ciudad.rutas_destino.all()
        
        # Eliminar rutas donde es origen
        for ruta in rutas_origen:
            db.session.delete(ruta)
        
        # Eliminar rutas donde es destino
        ...
        flash(f'Ciudad "{ciudad.nombre}" y sus rutas asociadas eliminadas exitosamente.', 'success')
    except Exception as e:
        db.session.rollback()
